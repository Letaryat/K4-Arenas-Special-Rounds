...
public class PluginK4ArenaOnlyHS : BasePlugin
{
    ...
    public override string ModuleName => "K4-Arenas Addon - Test";
    ...
    public override void OnAllPluginsLoaded(bool hotReload)
    {
        ...
        if (checkAPI != null)
        {
            RoundTypeID = checkAPI.AddSpecialRound("Test Round", 1, true, RoundStart, RoundEnd);
    ...
    public void RoundStart(List<CCSPlayerController>? team1, List<CCSPlayerController>? team2)
    {
        ...
        foreach (var p in team1){
            ...
            t1!.Add(p);
            var test = checkAPI!.GetPlayerWeaponPreferences(p);
            var test2 = checkAPI.GetArenaName(p);
            var penis = test["Rifle"];
            Server.PrintToChatAll($"TEST1: {test2}");
        }
        foreach(var p in team2){
            ...
            var test2 = checkAPI.GetArenaName(p);
            var penis = test["Rifle"];
            Server.PrintToChatAll($"TEST2: {test2}");
        }
    }
